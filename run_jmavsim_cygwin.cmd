...
"%CYGWIN_BASH%" -lc "export PATH=/home/%USERNAME%/.local/bin:/usr/local/bin:/usr/bin:'%JDK_CYG%/bin':'%ANT_CYG%/bin'; export JAVA_HOME='%JDK_CYG%'; export PYTHONUTF8=1; unset HEADLESS; export PX4_SIM_MODEL=jmavsim_iris; cd '%REPO_CYG%' && mkdir -p build/px4_sitl_cygwin39/rootfs && if [ ! -e build/px4_sitl_cygwin39/rootfs/jmavsim_run.sh ]; then ln -sf ../../../Tools/simulation/jmavsim/jmavsim_run.sh build/px4_sitl_cygwin39/rootfs/jmavsim_run.sh || cp Tools/simulation/jmavsim/jmavsim_run.sh build/px4_sitl_cygwin39/rootfs/jmavsim_run.sh; fi && chmod +x build/px4_sitl_cygwin39/rootfs/jmavsim_run.sh && build/px4_sitl_cygwin39/bin/px4.exe"
